...
foreach($messages as $key=>$value) {
    ...
    echo '<div class="comment-avatar col-md-1 col-sm-2 text-center pr-1" >';
        echo '<img class="mx-auto rounded img-fluid" style="border: 1px solid black;"  src="/fw/public/thumb/c_' .  $value['img'] .'" alt="avatar">';
        echo '</div>';
    echo '<div class="comment-content col-md-11 col-sm-10" >';
        echo '<h6 class="small comment-meta"><a href="/fw/user/userpage?userid=' .  $value['userid'] .'" target="blank" class="text-reset">' .  $value['firstname'] . ' ' . $value['lastname'] . '</a>' . ' <small class="text-white">' . ' ' . date('d.m H:i', $value['thetime']) . '</small></h6>';
...
<form method="post" action="/fw/messages/index"> 
...
<div class="list-group">
<form method="get" action="/fw/messages/index">
<?php
